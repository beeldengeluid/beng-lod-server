...
def get_uri(cat_type="PROGRAM", daan_id=None):
    if daan_id is None:
        return None
    model = DAANRdfModel
    url_parts = urlparse(model.NISV_DATA_NAMESPACE)
...
#     app = Flask(__name__)
#     cache.init_app(app, application_settings_oai)
#
#     # PROGRAM
...
#             ) in program_triples
#
#     assert (rdf_concept.itemNode, URIRef(model.NISV_SCHEMA_NAMESPACE + "hasSortDate"),
...
#             URIRef(model.IS_PART_OF_SERIES),
#             URIRef(get_uri(cat_type="SERIES", daan_id="2101909080260953431"))
#             ) in seasonTriples
#
#     producers = list(graph.objects(rdfConcept.itemNode, URIRef(model.NISV_SCHEMA_NAMESPACE + "hasProducer")))
#     assert len(producers) == 1
#     assert (producers[0], URIRef(model.NISV_SCHEMA_NAMESPACE + "hasRole"),
#             Literal("producent", datatype=XSD.string)) in list(graph.triples((producers[0], None, None)))
#     productionOrganisations = list(
#         graph.objects(producers[0], URIRef(model.NISV_SCHEMA_NAMESPACE + "hasOrganisation")))
#     assert len(productionOrganisations) == 1
#     assert (productionOrganisations[0], RDF.type, URIRef(model.NISV_SCHEMA_NAMESPACE + "Organisation")) in list(
...
#     mainTitles = list(graph.objects(rdfConcept.itemNode, URIRef(model.NISV_SCHEMA_NAMESPACE + "hasMainTitle")))
#     assert len(mainTitles) == 1
#     titleTexts = list(graph.objects(mainTitles[0], URIRef(model.NISV_SCHEMA_NAMESPACE + "hasTitleText")))
#     assert len(titleTexts) == 1
#     assert str(titleTexts[0]) == "Dilemma's rond leven en dood"
#     subjects = list(graph.objects(rdfConcept.itemNode, URIRef(model.NISV_SCHEMA_NAMESPACE + "hasSubject")))
#     expectedSubjects = ["hindoes", "donors", "euthanasie"]
#     for subject in subjects:
#         subjectTriples = list(graph.triples((subject, None, None)))
#         assert (subject, RDF.type, URIRef(model.NISV_SCHEMA_NAMESPACE + "Subject")) in subjectTriples
#         subjectLabel = graph.preferredLabel(subject)[0][1]
#         assert str(subjectLabel) in expectedSubjects
#     genres = list(graph.objects(rdfConcept.itemNode, URIRef(model.NISV_SCHEMA_NAMESPACE + "hasGenre")))
#     assert len(genres) == 1
#     for genre in genres:
#         genreTriples = list(graph.triples((genre, None, None)))
#         assert (genre, RDF.type, URIRef(model.NISV_SCHEMA_NAMESPACE + "Genre")) in genreTriples
...
#     creationDates = list(graph.objects(rdfConcept.itemNode, URIRef(model.NISV_SCHEMA_NAMESPACE + "hasCreationDate")))
#     assert len(creationDates) == 1
#     assert str(creationDates[0]) == "2008-04-11T21:30:09+00:00"
#     programmes = list(graph.objects(rdfConcept.itemNode, URIRef(model.NISV_SCHEMA_NAMESPACE + "isCarrierOf")))
...
#     carrierTypes = list(graph.objects(rdfConcept.itemNode, URIRef(model.NISV_SCHEMA_NAMESPACE + "hasCarrierType")))
...
#     rdfConcept = NISVRdfConcept(clip_json["GetRecord"]["record"]["metadata"]["entry"],
#                                 "LOGTRACKITEM",
#                                 application_settings_oai)
#     graph = rdfConcept.graph
#
...
#     cat_type = "PROGRAM"
#     assert (rdfConcept.itemNode,
#             URIRef(model.IS_PART_OF_PROGRAM),
...
# def test_storage_api_payload_to_rdf(application_settings_dm_flex, i_program_storage, i_season, i_series, i_carrier,
#                                     i_clip):
#     app = Flask(__name__)
#     cache.init_app(app, application_settings_dm_flex)
#
#     # PROGRAM
#     model = DAANRdfModel
#     rdfConcept = NISVRdfConcept(i_program_storage,
#                                 "PROGRAM",
#                                 application_settings_dm_flex)
#     graph = rdfConcept.graph
...
#
#     assert (rdfConcept.itemNode,
#             URIRef(model.NISV_SCHEMA_NAMESPACE + "hasSortDate"),
#             Literal("2019-03-26", datatype=XSD.date)
#             ) in programTriples
#     creators = list(graph.subjects(RDF.type, URIRef(model.NISV_SCHEMA_NAMESPACE + "Creator")))
...
# TODO: create a test for schema.org model
# def test_storage_api_payload_to_rdf(application_settings_dm_flex_sdo, i_program_storage, i_season, i_series, i_carrier,
#                                     i_clip):
