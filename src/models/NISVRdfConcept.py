...
class NISVRdfConcept:
    ...
    def __init__(self, metadata, concept_type, config, model):
        self.config = config
        ...
        self.graph.namespace_manager.bind("skos",
                                          SKOS)
        self.graph.namespace_manager.bind("gtaa",
                                          Namespace(self._model.GTAA_NAMESPACE))
        self.graph.namespace_manager.bind("non-gtaa",
                                          Namespace(self._model.NON_GTAA_NAMESPACE))
        ...
        for uri, rdfProperty in properties.items():
            ...
            if newPayload:
                ...
                for newPayloadItem in newPayload:
                    ...
                    elif rdfProperty["rangeSuperClass"] == self._model.ACTING_ENTITY or rdfProperty[
                            ...
                            if "origin" in concept and "value" in concept and "resolved_value" in concept:
                                if concept["resolved_value"] == newPayloadItem:
                                    ...
                                    if rdfProperty["range"] in self._model.NON_GTAA_TYPES:
                                        conceptNode = URIRef(self._model.NON_GTAA_NAMESPACE + concept["value"])
                                    else:
                                        conceptNode = URIRef(self._model.GTAA_NAMESPACE + concept["value"])
                                skosConcept = True
                        if not conceptNode:
