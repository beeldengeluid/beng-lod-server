...
if __name__ == "__main__":
    ...
    cleaned_data = [
        ["Role", "Wikidata identifier", "Wikidata label", "UM identifier", "UM label"]
    ]

    for file in os.listdir("util"):
        if (
            file.endswith(".csv")
            and file != "music_roles.csv"
            and file != "music_role_labels.csv"
        ):
            ...
            with open(os.sep.join(["util", file]), "r") as f:
                ...
                for row in file_reader:
                    ...
                    if wikidata_identifier:
                        nl_label, en_label = get_nl_and_en_label(
                            wikidata_identifier, wikidata_endpoint
                        )

                    if (
                        wikidata_identifier or um_identifier
                    ):  # only write rows with mappings
                        cleaned_row = [
                            row[0],
